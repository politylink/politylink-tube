...
const ClipPage = ({ data }) => {
  ...
  useEffect(() => {
    if (!playerRef.current) {
      ...
      playerRef.current = videojs(
        videoElement,
        getVideojsOptions(data.clipJson.video.url),
        onReady
      );
      videoRef.current.childNodes[0].childNodes[0].setAttribute(
        "playsinline",
        ""
      );
    }
  ...
  const highlightTranscript = (currentTime) => {
    let activeWordPosition = findActiveWordPosition(
