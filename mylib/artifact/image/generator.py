from dataclasses import dataclass
from logging import getLogger
from pathlib import Path

import cv2

from mylib.utils.constants import ImageSize

LOGGER = getLogger(__name__)

IMAGE_DSIZE_MAP = {
    ImageSize.SMALL: (160, 90),
    ImageSize.MEDIUM: (320, 180),
    ImageSize.LARGE: (640, 360)
}


@dataclass
class ImageGenerateRequest:
    time_sec: float = 0.
    size: ImageSize = None
    dsize: tuple = None  # (width, height)
    local_fp: Path = ''
    s3_fp: Path = ''
    overwrite: bool = False


class ImageGenerator:
    def __init__(self, m3u8_url: str, s3_client=None):
        self.cap = cv2.VideoCapture(m3u8_url)
        self.fps = self.cap.get(cv2.CAP_PROP_FPS)
        self.s3_client = s3_client

    def generate(self, request: ImageGenerateRequest):
        if request.local_fp and request.local_fp.exists() and not request.overwrite:
            LOGGER.debug(f'{request.local_fp} already exists')
            return cv2.imread(str(request.local_fp))

        if request.s3_fp:
            if not self.s3_client:
                raise ValueError(f'you need s3 client to upload to {request.s3_fp}')
            if not request.local_fp:
                raise ValueError(f'you need local file to upload to {request.s3_fp}')

        if not request.dsize and request.size:
            request.dsize = IMAGE_DSIZE_MAP[request.size]

        frame_number = int(self.fps * request.time_sec)
        self.cap.set(cv2.CAP_PROP_POS_FRAMES, frame_number)
        _, image = self.cap.read()
        if request.dsize:
            image = cv2.resize(image, dsize=request.dsize)

        if request.local_fp:
            request.local_fp.parent.mkdir(exist_ok=True, parents=True)
            cv2.imwrite(str(request.local_fp), image)
            LOGGER.debug(f'saved {request.local_fp}')
        if request.s3_fp:
            self.s3_client.upload_file(
                str(request.local_fp),
                'politylink',
                str(request.s3_fp),
                ExtraArgs={'ContentType': 'image/jpeg'}
            )
            LOGGER.debug(f'uploaded {request.local_fp} to {request.s3_fp}')

        return image
